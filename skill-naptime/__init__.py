...
from alice import AdaptIntent, AliceSkill, intent_handler
from alice.audio import wait_while_speaking
from alice.configuration.config import Configuration
from alice.messagebus.message import Message


class NapTimeSkill(AliceSkill):
    """Skill to handle alice speech client listener sleeping."""
    ...
    def initialize(self) -> None:
        ...
        self.add_event('alice.awoken', self.handle_awoken)
    ...
    def handle_awoken(self, _) -> None:
        """Handler for the alice.awoken message

        The message is sent when the listener hears 'Alice, Wake Up',
    ...
    def display_sleep_face(self) -> None:
        ...
        elif self.platform == "alice_mark_1":
    ...
    def perform_awaken(self) -> None:
        ...
        if self.platform != "unknown":
            self.bus.emit(Message('alice.volume.unmute',
    ...
    def enable_confirm_listening(self) -> None:
        """Patch active alice configuration to enable listening beep."""
        msg = Message('configuration.patch',
