"""Alice skill to pair a device to the Selene backend."""
import time
...
import alice.audio
from alice.api import DeviceApi, is_paired, check_remote_pairing
from alice.identity import IdentityManager
from alice.messagebus.message import Message
from alice.skills import intent_handler, AliceSkill
from alice.skills.intent_service import AdaptIntent


MARK_II = 'alice_mark_2'
ACTION_BUTTON_PLATFORMS = ('alice_mark_1', MARK_II)
...
def _stop_speaking():
    ...
    if alice.audio.is_speaking():
        alice.audio.stop_speaking()


class PairingSkill(AliceSkill):
    ...
    def initialize(self):
        ...
        self.add_event("alice.not.paired", self.not_paired)
        ...
        # If the device isn't paired catch alice.ready to report
        ...
        if not is_paired():
            self.add_event("alice.ready", self.handle_alice_ready)
    ...
    def _communicate_create_account_url(self):
        ...
        if not self.account_creation_requested:
            ...
            if self.gui.connected:
                ...
                self.enclosure.mouth_text("account.alice.ai      ")
            self.speak_dialog("create.account")
            alice.audio.wait_while_speaking()
            time.sleep(30)
    ...
    def _communicate_pairing_url(self):
        ...
        if self.gui.connected:
            ...
            self.enclosure.mouth_text("alice.ai/pair      ")
    ...
    def check_for_device_activation(self):
        ...
        device on account.alice.ai.  Activation is considered successful when
    ...
    def _handle_activation(self, login: dict):
        ...
        self.bus.emit(Message("alice.paired", login))
        ...
        self.bus.emit(Message("alice.device.show.idle"))
        self.reload_skill = True

    def _save_identity(self, login: dict):
        """Save this device's identifying information to disk.

        The user has successfully paired the device on account.alice.ai.
    ...
    def _end_pairing(self, error_dialog: str):
        ...
        self.bus.emit(Message("alice.mic.unmute", None))
        self._reset_pairing_attributes()
    ...
    def _restart_pairing(self, quiet: bool = False):
        ...
        self._reset_pairing_attributes()
        self.bus.emit(Message("alice.not.paired", data=dict(quiet=quiet)))
