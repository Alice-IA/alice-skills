Feature: Cancel Timers
  ...
  Scenario Outline: cancel a timer when only one timer is active
    ...
    Then "alice-timer" should reply with dialog from "cancelled-single-timer.dialog"
  ...
  Scenario Outline: cancel a timer when multiple timers are active using duration
    ...
    And multiple active timers
      ...
      | 3 minutes |
    When the user says "<cancel timer request>"
    Then "alice-timer" should reply with dialog from "ask-which-timer-cancel.dialog"
    And the user replies "1 minute"
    And "alice-timer" should reply with dialog from "cancelled-timer-named.dialog"
  ...
  Scenario Outline: attempt to cancel timer when there are no timers active
    ...
    When the user says "<cancel timer request>"
    Then "alice-timer" should reply with dialog from "no-active-timer.dialog"
  ...
  Scenario Outline: cancel a timer specifying duration
    ...
    And multiple active timers
      ...
      | 10 minutes |
    When the user says "<cancel timer request>"
    Then "alice-timer" should reply with dialog from "cancelled-timer-named.dialog"
  ...
  @xfail
  # Jira MS-61 https://alice.atlassian.net/browse/MS-61
  ...
  Scenario Outline: cancel all active timers
    ...
    And multiple active timers
      ...
      | 15 minutes |
    When the user says "<cancel timer request>"
    Then "alice-timer" should reply with dialog from "cancel-all.dialog"
