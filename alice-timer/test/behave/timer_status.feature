Feature: Timer Status
  ...
  Scenario Outline: status of a single timer
    ...
    Then "alice-timer" should reply with dialog from "time-remaining.dialog"
  ...
  Scenario Outline: status of named timer
    ...
    When the user says "<timer status request>"
    Then "alice-timer" should reply with dialog from "time-remaining-named.dialog"
  ...
  Scenario Outline: status of two timers
    ...
    And multiple active timers
      ...
      | 10 minutes |
    When the user says "<timer status request>"
    Then "alice-timer" should reply with dialog from "number-of-timers.dialog"
    And "alice-timer" should reply with dialog from "time-remaining-named.dialog"
    And "alice-timer" should reply with dialog from "time-remaining-named.dialog"
