...
class ConfigurationSkill(AliceSkill):
    ...
    def __init__(self):
        ...
        self.api = DeviceApi()
        self.config_hash = ''
        self.model_file = expanduser('~/.alice/precise/hey-alice.pb')
    ...
    def handle_use_precise_dev(self, message):
        from alice.configuration.config import (
    ...
    def handle_disable_precise_dev(self, message):
        ...
        for item in glob(expanduser('~/.alice/precise/precise-engine*')):
    ...
    def handle_set_listener(self, message):
        ...
        if module == 'precise':
            engine_folder = expanduser('~/.alice/precise/precise-engine')
    ...
    def handle_update_intent(self, message):
        try:
            self.bus.emit(Message('alice.skills.settings.update'))
            if self.update():
    ...
    def update(self):
        ...
        Reads remote configuration from the Alice backend and trigger
