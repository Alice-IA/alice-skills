...
from alice.api import DeviceApi, is_paired
from alice.skills.msm_wrapper import build_msm_config, create_msm
...
class SkillInstallerSkill(AliceSkill):
    _asm = None
    ...
    def asm(self):
        if self._asm is None:
            asm_config = build_asm_config(self.config_core)
            self._asm = create_asm(asm_config)
    ...
    def install(self, message):
        ...
        with self.handle_msm_errors(message.data['skill'], self.install_word):
            ...
            was_beta = is_beta(
                skill.name,
                self.asm.device_skill_state['skills']
            )
            ...
            if skill.is_local:
                self.asm.remove(skill)
                self.asm.install(skill, origin='voice')
            else:
                self.asm.install(skill, origin='voice')
    ...
    def update_skills_json(self):
        ...
        if upload_allowed and is_paired():
            try:
                DeviceApi().upload_skills_data(self.asm.device_skill_state)
            except Exception:
    ...
    def handle_list_skills(self, message):
        skills = [skill for skill in self.asm.all_skills if not skill.is_local]
        shuffle(skills)
    ...
    def handle_msm_errors(self, repo_name, action):
        try:
            ...
            self.log.error('ASM failed: ' + repr(e))
    ...
    def __marketplace_install(self, install_list):
        ...
        try:
            ...
            asm_skills = self.asm.all_skills
            # Remove skills not known to msm
            skills = [s for s in skills if s in [s.name for s in asm_skills]]
            # Remove already installed skills from skills to install
            installed_skills = [s.name for s in asm_skills if s.is_local]
            ...
            def install(name):
                ...
                s = self.asm.find_skill(name)
                try:
                    self.asm.install(s, origin='marketplace')
            ...
            result = self.asm.apply(install, skills)
    ...
    def __marketplace_remove(self, remove_list):
        ...
        try:
            ...
            installed_skills = [s.name for s in self.asm.all_skills if s.is_local]
            skills = [s for s in skills if s in installed_skills]

            self.log.info('Will remove {} from the marketplace'.format(skills))
            result = self.asm.apply(self.asm.remove, skills)
            return skills
        except Exception as e:
    ...
    def clean_author(self, skill):
        ...
        if skill.author == "aliceia":
            return "Alice IA"  # totally cheating, I know
        else:
            return skill.author
    ...
    def find_skill(self, param, local):
        ...
        try:
            return self.asm.find_skill(param)
            ...
            if names:
                ...
                if not response:
                    raise StopIteration
                return self.asm.find_skill(response, skills=skills)
            else:
