...
from alice.audio import wait_while_speaking
from alice.messagebus.message import Message
from alice.skills.core import AliceSkill, intent_handler
from alice.util import play_wav
from alice.util.parse import extract_number


ALSA_PLATFORMS = ['alice_mark_1', 'picroft', 'unknown']


class VolumeSkill(AliceSkill):
    """
    Control the audio volume for the Alice system

    Terminology:
       "Level" =  Alice volume levels, from 0 to 10
    ...
    def __init__(self):
        ...
        if self.config_core['enclosure'].get('platform') == 'alice_mark_1':
    ...
    def _clear_mixer(self):
        ...
        For alice_mark_1 do not reinstantiate the mixer.
        """
        platform = self.config_core['enclosure'].get('platform', 'unknown')
        if platform != 'alice_mark_1':
            self._mixer = None
    ...
    def initialize(self):
        ...
        self.add_event('alice.volume.increase',
                       self.handle_increase_volume)
        self.add_event('alice.volume.decrease',
                       self.handle_decrease_volume)
        self.add_event('alice.volume.mute',
                       self.handle_mute_volume)
        self.add_event('alice.volume.unmute',
    ...
    def _setvolume(self, vol, emit=True):
        ...
        # response to the alice.volume.set message.

        if emit:
            # Notify non-ALSA systems of volume change
            self.bus.emit(Message('alice.volume.set',
    ...
    def handle_max_volume(self, message):
        ...
        if speak_message:
            self.speak_dialog('max.volume')
            wait_while_speaking()
        self.bus.emit(Message('alice.volume.duck'))
    ...
    def _unmute_volume(self, message=None, speak=False):
        ...
        self.bus.emit(Message('alice.volume.unduck'))
    ...
    def __get_system_volume(self, default=50, show=False):
        ...
        if self.mixer:
            ...
            vol_msg = self.bus.wait_for_response(
                                Message("alice.volume.get", {'show': show}))
            if vol_msg:
